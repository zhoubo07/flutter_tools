...
/// @Title   可折叠的文本
/// @Author: zhoubo
/// @CreateDate:  2022年09月30日15:00:59
class FoldTextView extends StatefulWidget {
  final String text;

  /// 折叠时最大行数-在收起的状态下展示几行
  final int? maxLinesOnFold;

  /// 最多展示几行-在展开的状态下展示几行，
  /// 默认最多支持显示99行，要显示更多的行数需要手动传入
  final int? maxLinesOnFull;
  ...
  const FoldTextView({
    ...
    this.textStyle,
    this.maxLinesOnFull,
    this.keyColor = const Color(0xFF666666),
    this.shrinkText = '展开',
    this.expandText = '收起',
    this.onShrink,
    this.onExpand,
...
class _FoldTextViewState extends State<FoldTextView> {
  ...
  @override
  void didUpdateWidget(FoldTextView oldWidget) {
  ...
  void init() {
    ...
    if (widget.maxLinesOnFold != null && widget.maxLinesOnFold! > 0) {
      initValue();
    }
  }
  ...
  int? get _minLines => _isExpand ? _maxLines : widget.maxLinesOnFold;

  int get _maxLines => widget.maxLinesOnFull ?? 99;

  bool _isOverflow(List metrics, int? len) =>
      null == len ? false : metrics.length > len;
    ...
    Future.delayed(const Duration(), () {
      ...
      try {
        ...
        lineMetrics = List.generate(
          max(99, widget.maxLinesOnFull ?? 99),
          (index) => null,
        );
      }
      setState(() {
        _isOver = _isOverflow(lineMetrics, widget.maxLinesOnFold);
        if (_isOver) {
          _shrinkText = _calcShrinkText(
              lineMetrics, widget.maxLinesOnFold, _shrinkBtnText);
  ...
  List<LineMetrics> _paintText(String text, {double? maxWidth}) {
    ...
    if (maxWidth != null) {
      ...
      textPainter.layout(
          minWidth: textWidth ?? 0.0, maxWidth: textWidth ?? double.infinity);
    }
  ...
  String _calcShrinkText(
    ...
    if (null == len || len > lineMetrics.length) return widget.text;
  ...
  Widget _dispatch() {
    ...
    return Text(
      widget.text,
      maxLines: widget.maxLinesOnFold,
      style: _textStyle,
    );
  }
